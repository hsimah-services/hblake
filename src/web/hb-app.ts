export class HbApp extends HTMLElement {
  private main!: HTMLElement

  connectedCallback() {
    this.innerHTML = `
      <div class="layout">
        <hb-header current-path="${window.location.pathname}"></hb-header>
  ...
  private renderContent() {
    ...
    const header = this.querySelector('hb-header')
    header?.setAttribute('current-path', path)

    switch (true) {
      case path === '/':
      default:
        this.main.innerHTML = '<hb-feed></hb-feed>'
        break
